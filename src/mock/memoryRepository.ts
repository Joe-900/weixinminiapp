/**
 * @file In-memory mock repository for frontend local mode
 * @description All data stored in memory, no network requests
 */
...
import type { MockRepository, PageResult } from './types'

export class MemoryRepository implements MockRepository {
  ...
  async listTasksForUser(openid: string, groupIds: string[]): Promise<ReadingTask[]> {
    const memberGroupIds = new Set(
      groupIds.filter((groupId) => Array.from(this.members.values()).some(
        (member) => member.groupId === groupId && member.openid === openid,
      )),
    )
    return Array.from(this.tasks.values()).filter(
      (task) => memberGroupIds.has(task.groupId) && task.status === 'published',
    )
  }
