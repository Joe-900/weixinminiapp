...
- 阶段一已提交：`7b5f638 接入 ShuaiAPI 文本伴读`。Provider 配置示例不含密钥，兼容请求和错误分类已由 stub 测试覆盖；真实外部请求仍需在云函数运行时注入环境变量后执行。
- 阶段二已提交：`5bad391 完善伴读图片上传与多模态请求`。当前图片、历史图片、图片-only 默认问题、微信文件类型校验和存储错误已覆盖。
- 阶段三代码已完成：图书标签字段、管理员权限、标签过滤、页面展示和边界测试已通过；本提交将包含该阶段的中文 Git 提交。
